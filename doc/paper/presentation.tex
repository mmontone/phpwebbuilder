...
\item Por último, para \PITS, \PWB \ presenta una serie de características displonibles en otros lenguajes y plataforma pero no existentes en PHP. Estas son una implementación de Eventos (sección \ref{sub-events}) (especialmente útiles para mantener la vista actualizada), Weak References (sección \ref{sub-weak}), creación de DSLs (sección \ref{sub-phpcc}), Macros (sección \ref{sub-macros}) y Mixins(sección \ref{sub-mixins}).
