...
function updatePage(text, xml) {
    ...
    while(fc!= null && fc.nodeName!='ajax'){fc = fc.nextSibling;}
...
function all_updatePage(text, actions) {
   ...
   for (; i< actions.length; i++) {
	  var str = "ajax_" + actions[i].tagName.toLowerCase() + "(actions[i]);";
	  eval(str);
   }
   return true;
}
...
function ajax_setatt(action) {
    ...
    if (action.childNodes[1].hasChildNodes()){
	    var value = unescapeHTML(action.childNodes[1].firstChild.data);
	} else {
		var value = "";
	}
