...
function sendUpdate(update) {
    ...
    postToAjax(createSubmitData(),"Action.php",updatePage);
    updates = new Object();
}
...
function uploadFile(fileelement){
    ...
    var based = document.getElementById("app").getAttribute('basedir');
    var pwbu = document.getElementById("app").getAttribute('pwb_url');
    ifr.src=pwbu+'/lib/uploadFile.php?filenamefield='+fileelement+
    '&basedir='+based+'&app='+document.getElementById("app").getAttribute('app');
...
function initialize_history(){
    ...
    if (dhtmlHistory.isFirstLoad() || dhtmlHistory.getCurrentLocation() == "") {
        dhtmlHistory.add(document.getElementById("app").getAttribute("bookmark"));
    }
 }
...
function historyChange(newLocation, historyData) {
	if (dhtmlHistory.getCurrentLocation() == ""){
	    ...
	    sendUpdate(new Update('bookmark', newLocation));
    }
}
...
addOnUpdate(function(){
	var att = document.getElementById("app").getAttribute("push");
   if (att != null){
		push(att);
   }
});
...
function load_bookmark(hash){
    ...
    sendUpdate(new Update('bookmark', hash));
}
