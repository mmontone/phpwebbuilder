      ...
      var http = getHTTPObject();
       ...
       http.onreadystatechange = function () {
                       if (http.readyState==4) {
                            loadingStop();
                       		if (inIE()) {
                       				if (http.responseXML.firstChild == null || !func(http.responseText, http.responseXML))
			                             ajaxError(http.responseText);
	    		               }
	            		       else {
                        		   if (http.responseXML == null || !func(http.responseText, http.responseXML))
		                             ajaxError(http.responseText);
        		               }
						}
               };
...
function updatePage(text, xml) {
    if (inIE()) {
        ...
        return opera_updatePage(text,xml);
    } else {
...
function opera_updatePage(text, xml) {
    var acts = xml.firstChild.nextSibling.nextSibling;
    var actions = acts.childNodes;
    var i=0;
    for (; i< actions.length; i++) {
	  var str = "ajax_" + actions[i].tagName + "(actions[i]);";
