...
function createSubmitForm() {
    ...
    appendUpdate(form,'pwb_config',$F('pwb_config'));
...
function createSubmitData() {
    ...
    str = str + "&"+'pwb_config'+$F('pwb_config');
    for (var target in updates) {
...
function componentDblClicked(comp) {
    sendEvent('dblclick', comp);
    return false;
}
...
function openWindow(pos, params){
	...
	window.open(document.getElementById("app").getAttribute("action")+'&app='+document.getElementById("app_class").getAttribute("value")+'&date='+new Date().getTime()+'&window='+pos,pos, params);
}

function getWindowByName(name){
	var rw = getRootWindow();
	return rw.windows[name];
}

Event.observe(window, 'unload', function(){if (window.name!='') {
		getRootWindow().enqueueUpdate('window', 'window-'+window.name);
		getRootWindow().enqueueUpdate('event', 'closeWindow');
		getRootWindow().sendUpdate('event_target', 'app');
	}});

function refreshWindows(){
	for(var i = 0; i<arguments.length;i++){
		var win = getWindowByName(arguments[i]);
		if (win.name==arguments[i] || arguments[i]=='root') win.refresh();
	}
}
...
var contextMenus= [];
if (window.opener){ //On a subwindow
	window.root=window.opener.root;
	window.root.windows[window.name] = window;
} else {
	window.root=window;
	window.windows=new Array();
	window.windows["root"]=window;
}


function autocomplete(elemid){
    try{
    new Ajax.Autocompleter(elemid+'-displayText', elemid+'-listDisplay', $F('pwb_url')+"lib/autocomplete.php",
        {paramName:'value',afterUpdateElement: function(input, selected){
                try{
                        $(elemid+'-elementId').value=selected.value;
                        enqueueChange($(elemid+'-elementId'), inputGetValue);
                } catch(e){}
        },parameters: 'basedir='+$F('basedir')+'&app='+$F('app_class')+'&component='+elemid});
    }catch(e){}
}
