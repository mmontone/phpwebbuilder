...
function openWindow(pos){
	addOnLoad(function(){
		window.open(document.getElementById("app").getAttribute("action")+'&app='+document.getElementById("app_class").getAttribute("value")+'&window='+pos,pos);
	});
}
...
function refreshWindows(){
	for(var i = 0; i<arguments.length;i++){
		getWindowByName(arguments[i]).refresh();
	}
}