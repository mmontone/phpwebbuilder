...
function start_uploading(fieldname, filename){
	var movf = document.getElementById(fieldname);
	var newf = document.createElement("span");
	newf.appendChild(document.createTextNode("uploading "+filename+"..."));
	movf.parentNode.insertBefore(newf, movf.nextSibling);
	movf.style.visibility='hidden';
}
function end_uploading(fieldname, filename){
	var text = document.getElementById(fieldname).nextSibling;
	text.parentNode.replaceChild(document.createTextNode("uploaded "+filename), text);
}

function error_uploading(fieldname, filename){
	var text = document.getElementById(fieldname).nextSibling;
	text.parentNode.replaceChild(document.createTextNode("error uploading "+filename), text);
}
